...
By default all scout beans use a predefined mechanism to create instances as learned in the sections before. This mechanism can be modified for each bean to implement custom bean instance creation logic. For this the bean must be annotated with a producer as shown in listing <<lst-BeanProducer>>.

[[lst-BeanProducer, bean producer]]
[source,java,indent=0]
.A bean with custom instanciation logic
----
include::{codedir}/org.eclipse.scout.docs.snippets/src/main/java/org/eclipse/scout/docs/snippets/PlatformSnippet.java[tags=BeanProducer]
----
