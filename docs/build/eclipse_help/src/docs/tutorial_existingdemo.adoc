=== Import the Scout Demo Applications Tutorial
:doctype: article
...
===== Import the Scout demo projects
...
The [wizard]_Eclipse Importer_ contains the folder hierarchy [folder]_Eclipse Projects_, [folder]_<User>_, [folder]_Scout_ and [folder]_Scout Demo App_ as shown in <<img-oomph_exiting_projects1>>.
...
In case you have used already used Oomph for other purposes, this step might look different or empty (for example when all variables have been set in a previous Oomph session).
...
And as always, the Scout Team welcomes pull requests with improvements.
